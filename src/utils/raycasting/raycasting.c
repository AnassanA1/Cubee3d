...
/*   Updated: 2026/02/19 11:36:48 by msidry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_wallp(t_container *ref, t_ray *ray, t_wall *wall, int x, int line_height, double perpen_dis);
...
void    cast_ray(t_container *ref, int x)
{
    ...
    draw_wallp(ref, &ray, &wall, x, calculate_line_height(perpen_dis, ref->display.height), perpen_dis);
}
...
void    init_dda(t_container *ref, t_ray *ray, t_dda *data)
{
    ...
    if (ray->dir.x < 0)
        data->side_dist_x = (ref->player.pos.x - ray->map_x) * data->delta_dist_x;
    else
        data->side_dist_x = (ray->map_x + 1.0 - ref->player.pos.x) * data->delta_dist_x;
    if (ray->dir.y < 0)
        data->side_dist_y = (ref->player.pos.y - ray->map_y) * data->delta_dist_y;
    else
        data->side_dist_y = (ray->map_y + 1.0 - ref->player.pos.y) * data->delta_dist_y;
}
...
mlx_image_t *get_wall_texture(t_container *ref, int side, int step_x, int step_y)
{
