...
static void init_mlx(t_container *ref)
{
    ref->display.mlx = mlx_init(ref->display.width, ref->display.height, WIN_TITLE, true);
    if (!ref->display.mlx)
    {
...
static void init_img(t_container *ref)
{
    ref->display.img = mlx_new_image(ref->display.mlx, ref->display.width, ref->display.height);
    if (!ref->display.img)
    {
        ft_putstr_fd(ERROR_MLX, STDERR_FILENO);
        game_destroy(ref);
